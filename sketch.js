...
var pirateLaughSound, waterSound, Cannonexplosion, backgroundMusic;
  ...
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      if (boats[i]) {
        ...
        if(collision.collided && !boats[i].isNotBroken)
        {
            isGameOver = true;
            if(!pirateLaughSound.isPlaying() && isLaughing === false)
            {
              ...
              pirateLaughSound.setVolume(0.2);
              isLaughing = true;
            }
            gameOver();
        }
